...
def compute_mean_depth_loss(depth, target_mean=10.0, tolerance=0.2):
    """
    Compute mean depth loss with tolerance band
    """
    current_mean = depth.mean()
    # Only penalize if mean is outside tolerance range
    upper_bound = target_mean * (1 + tolerance)
    lower_bound = target_mean * (1 - tolerance)

    above_loss = torch.relu(current_mean - upper_bound)
    below_loss = torch.relu(lower_bound - current_mean)

    return 0.05 * (above_loss + below_loss) / target_mean
...
def compute_disparity_range_loss(disp, min_depth=0.5, max_depth=80.0):
    """
    Stronger range enforcement
    """
    min_disp = 1.0 / max_depth  # ~0.017
    max_disp = 1.0 / min_depth  # ~0.67

    below_min = torch.relu(min_disp - disp)
    above_max = torch.relu(disp - max_disp)

    # Add distribution loss to encourage spread
    hist = torch.histc(disp, bins=20, min=min_disp, max=max_disp)
    hist = hist / hist.sum()
    entropy = -(hist * torch.log(hist + 1e-10)).sum()

    return (below_min + above_max).mean() - 0.1 * entropy
...
class LossWeightScheduler:
    def __init__(self,
                 max_smoothness_weight=0.000005,
                 max_mean_depth_weight=0.1,
                 ramp_up_epochs=5):
        self.max_smoothness_weight = max_smoothness_weight
        self.max_mean_depth_weight = max_mean_depth_weight
        self.ramp_up_epochs = ramp_up_epochs

    def get_weights(self, epoch):
        """
        Returns scheduled weights for losses based on training epoch
        Gradually increases from 10% to 100% of max weight over ramp_up_epochs
        """
        # Calculate ramp-up factor (0.1 to 1.0)
        ramp_factor = min(1.0, 0.1 + 0.9 * (epoch / self.ramp_up_epochs))

        return {
            'smoothness': self.max_smoothness_weight * ramp_factor,
            'mean_depth': self.max_mean_depth_weight * ramp_factor
        }


def compute_enhanced_smoothness_loss(depth, image):
    """
    Enhanced edge-aware smoothness with stronger edge preservation
    """
    # Depth gradients
    depth_dx = torch.abs(depth[:, :, :, :-1] - depth[:, :, :, 1:])
    depth_dy = torch.abs(depth[:, :, :-1, :] - depth[:, :, 1:, :])
    ...
    edge_x = torch.sigmoid(50.0 * image_dx)  # Sharper edge detection
    edge_y = torch.sigmoid(50.0 * image_dy)

    # Dual-term weighting for better edge preservation
    weights_x = torch.exp(-150.0 * image_dx) * (1.0 - edge_x)
    weights_y = torch.exp(-150.0 * image_dy) * (1.0 - edge_y)
...
def compute_improved_smoothness_loss(depth, image):
    """
    Improved edge-aware and scale-aware smoothness loss
    """
    # Get gradients for depth
    depth_dx = torch.abs(depth[:, :, :, :-1] - depth[:, :, :, 1:])
    depth_dy = torch.abs(depth[:, :, :-1, :] - depth[:, :, 1:, :])

    # Get gradients for image
    image_dx = torch.mean(torch.abs(image[:, :, :, :-1] - image[:, :, :, 1:]), 1, keepdim=True)
    image_dy = torch.mean(torch.abs(image[:, :, :-1, :] - image[:, :, 1:, :]), 1, keepdim=True)

    # Normalize depth gradients by local depth values (scale-awareness)
    depth_dx = depth_dx / (depth[:, :, :, :-1] + 1e-6)
    depth_dy = depth_dy / (depth[:, :, :-1, :] + 1e-6)

    # Improved edge-awareness with sharper falloff and stronger edge detection
    weights_x = torch.exp(-150.0 * image_dx) * (1.0 - torch.exp(-3.0 * image_dx))
    weights_y = torch.exp(-150.0 * image_dy) * (1.0 - torch.exp(-3.0 * image_dy))
...
def compute_multi_scale_smoothness(depth, image, scales=[1, 2, 4]):
    """
    Apply smoothness loss at multiple scales for better global consistency
    """
    total_loss = 0
    weight = 1.0
    ...
    for scale in scales:
        if scale > 1:
            scaled_depth = F.interpolate(depth, scale_factor=1 / scale, mode='bilinear', align_corners=True)
            scaled_image = F.interpolate(image, scale_factor=1 / scale, mode='bilinear', align_corners=True)
        else:
            scaled_depth = depth
            scaled_image = image

        loss = compute_enhanced_smoothness_loss(scaled_depth, scaled_image)
        total_loss += weight * loss
        weight *= 0.5  # Reduce weight for larger scales

    return total_loss


def compute_ground_plane_loss(depth):
    """
    Enforce smooth depth transitions along ground plane
    """
    # Assume bottom third of image is mostly ground
    ground_region = depth[:, :, -depth.shape[2] // 3:, :]

    # Compute vertical gradients in ground region
    ground_grad = torch.abs(
        ground_region[:, :, :-1, :] - ground_region[:, :, 1:, :]
    )

    # Normalize by depth values
    ground_grad = ground_grad / (ground_region[:, :, :-1, :] + 1e-6)

    return ground_grad.mean()


def compute_vertical_structure_loss(depth, image):
    """
    Preserve vertical structures like trees and poles
    """
    # Detect vertical edges in image
    image_dy = torch.mean(torch.abs(
        image[:, :, :-1, :] - image[:, :, 1:, :]
    ), 1, keepdim=True)

    vertical_edges = (image_dy > 0.1).float()

    # Compute depth consistency along vertical edges
    depth_dy = torch.abs(
        depth[:, :, :-1, :] - depth[:, :, 1:, :]
    )

    # Apply stronger consistency at vertical edges
    return (depth_dy * vertical_edges).mean()
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            smoothness_loss = compute_multi_scale_smoothness(depth, curr_image)
            ground_loss = compute_ground_plane_loss(depth)
            vertical_loss = compute_vertical_structure_loss(depth, curr_image)
            range_loss = compute_disparity_range_loss(disp)

            total_loss += 0.2 * range_loss  # Increased weight
            total_loss += 0.1 * ground_loss + 0.1 * vertical_loss
            ...
            # Add smoothness loss
            # Add weighted regularization losses
            total_loss += weights['smoothness'] * smoothness_loss
            total_loss += 0.5 * weights['mean_depth'] * compute_mean_depth_loss(depth)
